...
function createTextTexture(
  ...
  // Make canvas larger for better text quality
    ...
    this.program = new Program(this.gl, {
      ...
      vertex: `
        ...
        void main() {
          vUv = uv;
          vec3 p = position;
          p.z = (sin(p.x * 4.0 + uTime) * 1.2 + cos(p.y * 2.0 + uTime) * 1.2) * (0.08 + uSpeed * 0.3);
          gl_Position = projectionMatrix * modelViewMatrix * vec4(p, 1.0);
        }
      `,
      fragment: `
...
class App {
  ...
  onTouchUp() {
    this.isDown = false;
    this.onCheck();
    // Resume auto-scrolling after 2 seconds of no interaction
  ...
  onWheel(e: Event) {
    ...
    this.scroll.target += delta > 0 ? this.scrollSpeed : -this.scrollSpeed;
    this.onCheckDebounce();
    // Resume auto-scrolling after 3 seconds of no wheel interaction
    this.interactionTimeout = window.setTimeout(() => {
      this.userInteracting = false;
    }, 3000);
  }
